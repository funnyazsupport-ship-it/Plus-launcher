...
const DEFAULTS = {
  ...
  aiCrashHelp: true,
  // Разрешать помощнику самому создавать сборки и ставить моды.
  // Даже когда включено, каждое действие он всё равно спрашивает отдельно —
  // это выключатель самой возможности, а не замена подтверждению.
  aiActions: true,
  ...
  friends: [],
  // Свои сервера: обычные, не про друзей. Попадают в тот же список внутри игры,
  // но живут здесь, а не в сборке — переустановка сборки их не уносит.
  servers: [],
