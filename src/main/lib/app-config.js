...
module.exports = {
  ...
  // Сервер, через который друзья попадают в чужой мир. Человеку он не показывается:
  // роутер не пускает внутрь никого, и без такого посредника игра вдвоём не заводится.
  // Ключ — пропуск на сам сервер, а не пароль от ника: пароли у каждого свои.
  // Лежит он в embedded-key.js, которого нет в git: репозиторий открытый,
  // и выложенный там ключ нашли бы поисковые роботы за сутки.
  relayHost: '89.168.120.170',
  relayPort: 7000,
  get relayKey() { return embeddedKey ? embeddedKey('relay') : ''; },
